...
class CmpSin extends VoltageElm{
    ...
    public CmpSin(int xx, int yy) { super(xx, yy, WF_AC);
   createSlider();
    }
                public CmpSin(int xa, int ya, int xb, int yb, int f,
            	       StringTokenizer st) {
	...
	int getDumpType() { return 424; }
	 double getVoltage() {
	     ...
	     double cycle = 1/f;
	     double dutycycle = 1/(f); 
	     NMUL = 1/f * numberofcycle;
	     NMISS = MISSINGTEETH * 1/f;
	     acycle = (NMUL + NMISS)/2;
	     double current_time_in_cycle = sim.t%acycle;
	     if(current_time_in_cycle >0 && current_time_in_cycle< (0+ dutycycle) ) datareturn = amplitude*Math.sin(2*pi*sim.t*f );
	     else if(current_time_in_cycle >3*cycle && current_time_in_cycle< (3*cycle+ dutycycle) ) datareturn =amplitude*Math.sin(2*pi*sim.t*f );
	     else if(current_time_in_cycle >5*cycle && current_time_in_cycle< (5*cycle+ dutycycle) ) datareturn = amplitude*Math.sin(2*pi*sim.t*f );
	     else if(current_time_in_cycle >6*cycle && current_time_in_cycle< (6*cycle+ dutycycle) ) datareturn = amplitude*Math.sin(2*pi*sim.t*f );
	     else if(current_time_in_cycle >9*cycle && current_time_in_cycle< (9*cycle+ dutycycle) ) datareturn = amplitude*Math.sin(2*pi*sim.t*f );
	     else if(current_time_in_cycle >12*cycle && current_time_in_cycle< (12*cycle+ dutycycle) ) datareturn = amplitude*Math.sin(2*pi*sim.t*f );
	     else if(current_time_in_cycle >15*cycle && current_time_in_cycle< (15*cycle+ dutycycle) ) datareturn = amplitude*Math.sin(2*pi*sim.t*f );
	     else datareturn =0;
